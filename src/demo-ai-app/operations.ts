...
export const createTask: CreateTask<CreateTaskInput, Task> = async (
  ...
  const { description, alerteId } = ensureArgsSchemaOrThrowHttpError(
  ...
  if (alerteId) {
    await context.entities.Alerte.update({
      where: { id: BigInt(alerteId) },
      data: {
...
export const updateTask: UpdateTask<UpdateTaskInput, Task> = async (
  ...
  if (isDone && task.alerteId) {
    const unresolvedTasks = await context.entities.Task.findMany({
      where: {
        alerteId: task.alerteId,
        isDone: false,
      },
    });
